...
class AspectJAutoProxyBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {		// <aop:aspectj-autoproxy>
		// 01、AopNamespaceUtils 注册 AnnotationAwareAspectJAutoProxyCreator
	...
	private void extendBeanDefinition(Element element, ParserContext parserContext) {
		// 01、beanDef = {RootBeanDefinition@2217} "Root bean: class [AnnotationAwareAspectJAutoProxyCreator]"
		BeanDefinition beanDef = parserContext.getRegistry().getBeanDefinition(AopConfigUtils.AUTO_PROXY_CREATOR_BEAN_NAME);
		// 02、处理子标签 <aop:include/>，TODO：指定 @Aspect 类，支持正则表达式，符合该表达式的切面类才会被应用
		// 解析 <aop:include /> 添加到 AnnotationAwareAspectJAutoProxyCreator.includePatterns
